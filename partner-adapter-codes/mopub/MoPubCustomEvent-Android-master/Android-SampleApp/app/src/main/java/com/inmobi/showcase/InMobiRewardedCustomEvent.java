...
public class InMobiRewardedCustomEvent extends CustomEventRewardedVideo implements
	...
	protected boolean checkAndInitializeSdk(@NonNull Activity activity,
			@NonNull Map<String, Object> arg1, @NonNull Map<String, String> serverExtras)
					throws Exception {

		if (activity == null) {
			Log.w(TAG, "Activity passed is Null. Returning error!");
			MoPubRewardedVideoManager
					.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class, getAdNetworkId(), MoPubErrorCode.NO_FILL);
			return false;
		}
	...
	protected void showVideo() {
		if(this.hasVideoAvailable()){
			...
			MoPubRewardedVideoManager.onRewardedVideoPlaybackError(InMobiRewardedCustomEvent.class, placementId, MoPubErrorCode.VIDEO_PLAYBACK_ERROR);
		}
	}
	...
	public void onAdLoadFailed(InMobiInterstitial arg0,
		...
		if (arg1.getStatusCode() == StatusCode.INTERNAL_ERROR) {
			MoPubRewardedVideoManager
			.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class, getAdNetworkId(), MoPubErrorCode.INTERNAL_ERROR);
		} else if (arg1.getStatusCode() == StatusCode.REQUEST_INVALID) {
			MoPubRewardedVideoManager
			.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class, getAdNetworkId(), MoPubErrorCode.ADAPTER_CONFIGURATION_ERROR);
		} else if (arg1.getStatusCode() == StatusCode.NETWORK_UNREACHABLE) {
			MoPubRewardedVideoManager
			.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class, getAdNetworkId(), MoPubErrorCode.NETWORK_INVALID_STATE);
		} else if (arg1.getStatusCode() == StatusCode.NO_FILL) {
			MoPubRewardedVideoManager
			.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class,getAdNetworkId(), MoPubErrorCode.NO_FILL);
		} else if (arg1.getStatusCode() == StatusCode.REQUEST_TIMED_OUT) {
			MoPubRewardedVideoManager
			.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class, getAdNetworkId(), MoPubErrorCode.NETWORK_TIMEOUT);
		} else if (arg1.getStatusCode() == StatusCode.SERVER_ERROR) {
			MoPubRewardedVideoManager
			.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class, getAdNetworkId(), MoPubErrorCode.SERVER_ERROR);
		} else {
			MoPubRewardedVideoManager
			.onRewardedVideoLoadFailure(InMobiRewardedCustomEvent.class, getAdNetworkId(), MoPubErrorCode.UNSPECIFIED);
		}
	}
